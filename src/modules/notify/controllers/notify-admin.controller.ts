...
export class NotifyController {
  ...
  async markAsRead(@Body() data: IdDto, @CurrentUser() user: UserDto) {
    return await this.service.markAsRead(data, user);
  }
  ...
  @UseGuards(JwtAuthGuard, RoleGuard, PermissionGuard)
  @RequireRoles('ADMIN')
  @RequirePermissions(['NOTIFICATION:DEACTIVATED'])
  @ApiOperation({ summary: 'Xóa mềm thông báo' })
  @Post('deactivate')
  async deactivate(@Body() data: IdDto) {
    return await this.service.deactivate(data);
  }
  ...
  async deactivateTemplate(@Body() data: IdDto) {
    return await this.service.deactivateTemplate(data);
  }
