...
export class AuthController {
  ...
  @Post('login/google')
  async loginWithGoogle(@Body() data: GoogleLoginDto, @Req() req: Request) {
    const ipAddress = req.ip || req.socket?.remoteAddress;
    const userAgent = req.headers?.['user-agent'];
    return await this.service.loginWithGoogle(data, { ipAddress, userAgent });
  }

  @Post('login/facebook')
  async loginWithFacebook(@Body() data: FacebookLoginDto, @Req() req: Request) {
    const ipAddress = req.ip || req.socket?.remoteAddress;
    const userAgent = req.headers?.['user-agent'];
    return await this.service.loginWithFacebook(data, { ipAddress, userAgent });
  }
  ...
  @Post('verify-otp')
  async verifyLoginOtp(@Body() data: VerifyLoginOtpDto, @Req() req: Request) {
    const ipAddress = req.ip || req.socket?.remoteAddress;
    const userAgent = req.headers?.['user-agent'];
    return await this.service.verifyLoginOtp(data, { ipAddress, userAgent });
  }
