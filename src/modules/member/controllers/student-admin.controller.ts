...
export class MemberController {
  ...
  @RequireRoles('ADMIN')
  @RequirePermissions(['STUDENT:VIEW_LIST'])
  ...
  @UseGuards(JwtAuthGuard, RoleGuard, PermissionGuard)
  @RequireRoles('ADMIN')
  @RequirePermissions(['STUDENT:VIEW_DETAIL'])
  ...
  @UseGuards(JwtAuthGuard, RoleGuard, PermissionGuard)
  @RequireRoles('ADMIN')
  @RequirePermissions(['STUDENT:CREATED'])
  ...
  @UseGuards(JwtAuthGuard, RoleGuard, PermissionGuard)
  @RequireRoles('ADMIN')
  @RequirePermissions(['STUDENT:EDITED'])
  ...
  @UseGuards(JwtAuthGuard, RoleGuard, PermissionGuard)
  @RequireRoles('ADMIN')
  @RequirePermissions(['STUDENT:DEACTIVATED'])
  ...
  @UseGuards(JwtAuthGuard, RoleGuard, PermissionGuard)
  @RequireRoles('ADMIN')
  @RequirePermissions(['STUDENT:ACTIVATED'])
  ...
  @ApiOperation({ summary: 'Lấy danh sách thống kê chứng chỉ của thành viên' })
  @Post('cert-stat/my-stats')
  async getMyCertStats(@CurrentUser() user: UserDto) {
    const memberId = user.memberId as string;
    return await this.service.findCertStatsByMember(memberId);
  }

  @ApiOperation({ summary: 'Kiểm tra quyền truy cập' })
  @Post('check-access')
  async checkAccess(
    @CurrentUser() user: UserDto,
    @Body() data: { itemType: string; itemId?: string },
  ) {
    return await this.service.checkAccessRight(
